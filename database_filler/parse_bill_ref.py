import sys
sys.path.insert(1, '../')
from get_data_files_to_parse import get_all_data_paths
import json
#NOTE: THIS IS USING PARSE SPONSOR CODE AS BASE
import os, csv

from sqlalchemy import create_engine
from sqlalchemy.orm import scoped_session, sessionmaker


engine = create_engine('sqlite:///../political_db.db', echo=True)
db = scoped_session(sessionmaker(bind=engine))

relative_congress_loc = "../../congress/data/"

def main():


    engine = create_engine('sqlite:///../political_db.db', echo=True)
    meta = MetaData()

    bill_ref = Table(
        'politician_term', meta,
        Column('id', Integer, primary_key=True),
        Column('to_bill', Integer),
        Column('from_bill', Integer),
        sqlite_autoincrement=True
    )


    meta.create_all(engine)
    conn = engine.connect()
    conn.execute(bill_ref.delete())
    conn.execute(bill_ref.delete())

    files = get_all_data_paths()

    for f in files:
        path = relative_congress_loc + f
        print(f)
        path = os.path.abspath(path)
        print(path)
        with open(path) as x:
            data = json.load(x)
            
            to_bill = data['bill_id']

            for related in data['related_bills']:
                to_bill = related['bill_id']

                ins = bill_ref.insert().values(to_bill=to_bill, to_bill=to_bill)
                db.execute(ins)

    db.commit()


#just making sure this works
if __name__ == "__main__":
    main()
